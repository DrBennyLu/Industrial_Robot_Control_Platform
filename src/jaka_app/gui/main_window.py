...
class MainWindow(QWidget):
    ...
    def _build_ui(self) -> None:
        self.setWindowTitle("JAKA Control (PyQt5)")
    ...
    def _page_status(self) -> QWidget:
        ...
        lay.addRow("errcode", self.lbl_err)
        lay.addRow("estop", self.lbl_estop)
        lay.addRow("power/servo", self.lbl_power)
        lay.addRow("enabled", self.lbl_enable)
        lay.addRow("rapidrate", self.lbl_rapid)
        lay.addRow("inpos", self.lbl_inpos)
        lay.addRow("program_state", self.lbl_prog)
        lay.addRow("logic_line", self.lbl_line)
        lay.addRow("loaded_file", self.lbl_file)
        lay.addRow("current_step (flow)", self.lbl_step)
        lay.addRow("run_state", self.lbl_run_state)
        lay.addRow("cycle_total", self.lbl_cycle_total)
        lay.addRow("cycle_ok", self.lbl_cycle_ok)
        lay.addRow("cycle_ng", self.lbl_cycle_ng)
        lay.addRow("yield(%)", self.lbl_yield)
        lay.addRow("last_cycle(s)", self.lbl_last_cycle)
        lay.addRow("avg_cycle(s)", self.lbl_avg_cycle)
        lay.addRow("best_cycle(s)", self.lbl_best_cycle)
        lay.addRow("fail_reason", self.lbl_fail_reason)
        lay.addRow("last_error", self.lbl_last_error)
        return p
